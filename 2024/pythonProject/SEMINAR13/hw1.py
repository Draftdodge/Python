...
class NegativeValueError(BaseException):
    def __init__(self, size_, side):
        ...
        if side == 1:
            ...
            self.side = 'Высота'
    def __str__(self):
        return f'{self.side} должна быть положительной, а не {self.size_}'
...
class Rectangle:
    ...
    def __init__(self, width, height=None):
        ...
        if height is None:
            ...
            self.height = height
        if self.width <= 0:
            raise NegativeValueError(self.width, 1)
        if self.height <=0:
            raise NegativeValueError(self.height, 0)
